<%@ page import="spockintegration.Account; spockintegration.Transaction" %>
<html>
<head>
            ...
            <table>
                ...
                <tr class="prop">
                    ...
                    <td valign="top" class="value ${hasErrors(bean: transactionInstance, field: 'type', 'errors')}">
                        <g:select name="type" from="${spockintegration.TransactionType?.values()}" keys="${spockintegration.TransactionType?.values()*.name()}" value="${transactionInstance?.type?.name()}"/>
                    </td>
                </tr>

                <tr class="prop">
                    <td valign="top" class="name">
                        <label for="type"><g:message code="transaction.account.label" default="Account"/></label>
                    </td>
                    <td valign="top" class="value ${hasErrors(bean: transactionInstance, field: 'account', 'errors')}">
                        <g:select name="account.id" from="${Account.list()}" optionKey="id" value="${transactionInstance?.account?.id}"/>
                    </td>
                </tr>
                </tbody>
            </table>
